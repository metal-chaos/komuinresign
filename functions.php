...
if (!is_admin()) {
	...
	$ieua = $_SERVER['HTTP_USER_AGENT'];
		...
		wp_register_script( 'glitch', get_bloginfo('template_directory'). '/library/js/libs/imagesloaded.pkgd.min.js', array(), '', true );
...
function register_style() {
	wp_register_style('style', get_bloginfo('template_directory').'/style.css?20190225');
...
add_filter( 'comment_form_default_fields', function ( $fields ) {
	...
	if ( 'products' === $_post_type ) {
		for ( $i = 0; $i < 5; $i++ ) {
			$selector .= '<option value="' . $ratings[$i]['rate'] . '" ' . selected( $ratings[$i]['rate'], 3, false ) . '>' . $ratings[$i]['rate'] . ' : ' . $ratings[$i]['text'] . '</option>';
		}

		$fields['rating'] = '<p class="comment-form-rating"><label for="rating">評価 <span class="required">*</span></label> ' . '<select id="rating" name="product_rating" size="1">' . $selector . '</select></p>';
	}
